...
import { readResearchLearningRuns } from "../../../../lib/research-v3-store";
...
export async function GET(request: NextRequest) {
  ...
  if (!game || !Number.isInteger(limit) || limit < 1 || limit > 50) {
    return NextResponse.json(
      { error: "彩种或数量无效。" },
      { status: 400 },
    );
  }
  const learningRuns = await readResearchLearningRuns(game, limit);
  return NextResponse.json(
    { game, learningRuns },
